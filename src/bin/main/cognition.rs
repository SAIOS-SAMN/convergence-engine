...
pub fn think(entity: &mut Entity, payload: &str) -> String {
            ...
            if pairs.is_empty() || n == 0 {
                ...
                let receipt_hash_hex = if p.coherence >= Q::one() && !pairs.is_empty() {
                    ...
                    // entity never builds words from letters.
                    {
                        ...
                        // opcode=255 (value-level, no spatial remap). sigma = the transition map.
                        // opcode 255 is outside SpatialPrimitive range (0-9), so from_opcode
                        // returns None and the Thought::Genomic falls through to identity spatial.
    ...
    // ── Fiber expansion: DISABLED until perception fills m=2 coordinate ──
    // The expansion mechanism (Delta::expand_fiber, fiber_expansion_readiness)
    // is implemented and tested. But the perception pipeline doesn't encode
    // structural information into the second fiber coordinate yet. Expanding
    // m without perception support causes index-out-of-bounds panics in the
    // compositor (vocabulary.rs:239 — values vector length mismatch).
    // Re-enable when perception encodes adjacency/cohesion into m=2.
    // See docs/SESSION_30_BUILD_LOG.md for the expansion design.
    ...
    (entity.state_record.parent_id > 0 && !entity.state_record.composed_operators.is_empty()).then(|| {
        let reports_path = entity.dir.join("reports.bin");
        let mut buf: Vec<u8> = Vec::new();
        // Write each solved orbit with cocycles, operators on FIRST orbit only.
        // The ABSORB parser reads operators per-orbit-block. Writing them once
        ...
        for (idx, orbit) in entity.state_record.solved_puzzles.iter().take(32).enumerate() {
            ...
            ops.iter().take(n_ops).for_each(|op| {
                ...
                op.sigma.iter().take(255).for_each(|(fv, tv)| {
                    buf.extend_from_slice(&fv.to_le_bytes());
                    buf.extend_from_slice(&tv.to_le_bytes());
                });
            });
